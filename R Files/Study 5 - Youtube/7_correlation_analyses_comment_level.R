...
combined_data %>% ggplot(aes(x = topic_4)) + geom_density()
combined_data %>% ggplot(aes(x = neg)) + geom_density()

summary(lm(topic_1 ~ neg, data = combined_data))
summary(lm(topic_2 ~ neg, data = combined_data))
summary(lm(topic_3 ~ neg, data = combined_data))
summary(lm(topic_4 ~ neg, data = combined_data))

# Calculate correlation between variables
cor_matrix <- combined_data %>%
  select("subscribers", "likes",
    "replies", "view_count", "like_count", "comment_count", 
    "comment_type", 
    ...
    "political_implication", "criticism", "changing_definition_race", "topic_1", "topic_2", 
    "topic_3", "topic_4", "compound", "pos", "neu", "neg", "but_count"
  ) %>%
